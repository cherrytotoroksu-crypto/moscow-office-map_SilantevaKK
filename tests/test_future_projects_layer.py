    ...
    def test_confirmed_duplicates_are_not_in_active_layer(self):
        ...
        self.assertEqual(
            duplicate_ids,
            {
                ...
                "OBJ-0689",
                # 2026-09-15: КРТ-проект у Курского вокзала (Верхняя
                # Сыромятническая) — 3 черновых записи из разных листов
                # исходного xlsx для ОДНОГО реального проекта, найдено при
                # веб-верификации маркера "требует проверки" на карте;
                # OBJ-0785 оставлена канонической (подтверждённый девелопер
                # и официальная площадь 51040 кв.м)
                "OBJ-0263", "OBJ-0505",
                # 2026-09: массовая веб-верификация 166 записей без источников
                # (батчи параллельных агентов) — 2 новых дубля, найденных при
                # проверке: OBJ-0173 "МФК Сити 20" = OBJ-0213 "Sezar Tower"
                # (тот же адрес/проект под старым рабочим названием, адрес
                # OBJ-0213 исправлен на подтверждённый); OBJ-0362 "Шмитовский
                # проезд" = OBJ-0638 (тот же адрес/застройщик STONE/площадь
                # 200000, разные карточки одного проекта)
                "OBJ-0173", "OBJ-0362",
                # 2026-09-17: раунд 2 веб-верификации (362 записи с
                # источниками) — 2 новых явных дубля с идентичным
                # адресом/участком и совпадающей (или одинаково пустой)
                # GBA/застройщиком: OBJ-0557 "шоссе Энтузиастов, вл. 12"
                # = OBJ-0556 (тот же участок, GBA 43500 у обеих);
                # OBJ-0504 "проспект Вернадского" = OBJ-0503 (тот же
                # адрес "Вернадского, 6В", обе без GBA/застройщика)
                "OBJ-0557", "OBJ-0504",
                # 2026-09-17: точечная повторная проверка needs_review-записей
                # раунда 2 (порция 2) — OBJ-0497 "Офис Газпромбанка" = OBJ-0496
                # (тот же адрес 3-й Добрынинский пер., 4/2, тот же проект,
                # подтверждённая площадь 14000, а не 21500 как у OBJ-0497)
                "OBJ-0497",
                # 2026-09-17: точечная повторная проверка needs_review-записей
                # раунда 2 (порция 3) — OBJ-0688 "Многофункциональный офисный
                # комплекс" = OBJ-0556 (тот же адрес ш. Энтузиастов, вл. 12,
                # тот же проект ТЭН, GBA 43500 у обеих); OBJ-0676 "ТПУ
                # Рязанская" = OBJ-0523 "ТПУ Рязанский" (тот же ТПУ в
                # Нижегородском районе, GBA 58353 точное совпадение)
                "OBJ-0688", "OBJ-0676",
                # 2026-09-17: ручной разбор 26 дубль-кандидатов из red-flag
                # набора раунда 2 — найдены через сверку адресов внутри базы
                # (не через веб-поиск): OBJ-0233 = OBJ-0385 (идентичный адрес/
                # застройщик Академика Пилюгина, 24, Сумма Элементов, обе без
                # GBA); OBJ-0270 = OBJ-0183 (тот же адрес ул. 2-я Ямская, вл. 2).
                # OBJ-0339 "Север.Сити" изначально ошибочно смёржен с OBJ-0374
                # "Aurus" (совпадали адрес/застройщик/GBA=393264) — ОТКАЧЕНО:
                # пользователь подтвердил, что это разные проекты одного
                # застройщика на одном крупном участке, совпадение полей не
                # означает дубль. OBJ-0339 остаётся отдельной активной записью.
                "OBJ-0233", "OBJ-0270",
            },
        )
