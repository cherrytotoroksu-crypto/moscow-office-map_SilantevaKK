"""Apply the tenth reviewed wave of office-project lifecycle dates."""
...
QA_PATH = ROOT / "data" / "qa" / "gap_fill_official_projects_20260926_wave10.json"
...
def main():
    ...
    for row in rows:
        ...
        date_record["source"] = (
            item["building_source_history"]
            + " Current commissioning sources: "
            + ", ".join(item["sources"])
        )
        ...
        classifier_row["layer_qa_notes"] = (
            item["classifier_layer_history"]
            + " Gap fill verified "
            + qa["checked_on"]
            + ": "
            + item["decision"]
    ...
    if missing:
        raise ValueError(f"Tenth-wave targets not found: {missing}")
