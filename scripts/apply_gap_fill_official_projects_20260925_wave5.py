"""Apply the fifth reviewed wave of office-project dates and statuses."""
...
QA_PATH = ROOT / "data" / "qa" / "gap_fill_official_projects_20260925_wave5.json"


def _apply_expected(record, before, after, label):
...
def main():
    ...
    for row in rows:
        ...
        _apply_expected(row, item["before"], item["after"], item["canonical_project_id"])
        ...
        record["commission_year"] = item["after"]["input_year"]
        ...
        classifier_row["verification_status"] = item["decision"]
        classifier_row["confidence"] = "medium"
        classifier_row["classification_note"] = item["decision"]
        classifier_row["review_reason"] = (
            "Quarter-level precision and unresolved area-definition conflicts remain under review."
        )
        classifier_row["needs_review"] = True
        classifier_row["layer_qa_notes"] = item["decision"]

    missing = sorted(set(accepted) - set(updated))
    if missing:
        raise ValueError(f"Fifth-wave targets not found: {missing}")
