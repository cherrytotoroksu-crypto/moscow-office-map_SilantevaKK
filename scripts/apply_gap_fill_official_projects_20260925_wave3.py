"""Apply the third reviewed wave of office-project commissioning dates."""
...
QA_PATH = ROOT / "data" / "qa" / "gap_fill_official_projects_20260925_wave3.json"
...
def main():
    ...
    for row in rows:
        ...
        note = (
            f" Commissioning date verified {qa['checked_on']}: {item['decision']} "
    ...
    if missing:
        raise ValueError(f"Third-wave targets not found: {missing}")

    date_keys = {
        "proj-102": "парк легенд класс а",
        "proj-152": "mind",
        "proj-252": "бизнес-парк раменки (бывший — огни)",
    }
    for project_id, key in date_keys.items():
        ...
        quarter = after["input_quarter"]
        record["commission_q"] = (
            f"{after['input_year']}{1 + (quarter - 1) * 3:02d}" if quarter else None
        )
