"""Apply the eighth reviewed wave of office-project facts and lifecycle dates."""
...
QA_PATH = ROOT / "data" / "qa" / "gap_fill_official_projects_20260926_wave8.json"
...
def main():
    ...
    for row in rows:
        ...
        record["commission_q"] = item["commission_q"]
        record["commission_year"] = item["after"]["input_year"]
        source_parts = []
        if item.get("existing_date_source"):
            source_parts.append(item["existing_date_source"])
        source_parts.append("Verified sources: " + ", ".join(item["sources"]))
        record["source"] = " ".join(source_parts)
        ...
        classifier_row["confidence"] = "Высокий" if item["confidence"] == "high" else "Средний"
    ...
    if missing:
        raise ValueError(f"Eighth-wave targets not found: {missing}")
