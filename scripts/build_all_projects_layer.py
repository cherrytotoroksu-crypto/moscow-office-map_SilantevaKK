...
EXTRA_ALIASES = {
    ...
    # Продолжение аудита review_queue 2026-08-18: полная сверка всех 140
    # уникальных ключей верхнего уровня по data/lots_*.json (2021-2026)
    # против реестра — 3 ключа не совпали текстово, но совпадают по
    # адресу/площади/девелоперу с уже существующими записями (сам
    # canonical_name уже фиксирует более старое короткое имя в скобках
    # "бывший — ...").
    "proj-99": ["Магистральная ул., 12"],   # lots_202503.json, GBA/адрес совпадают
    "proj-237": ["Октябрьское поле"],        # lots_202603.json, площадь лота 11893.9 = GBA записи
    "proj-158": ["Солид"],                   # lots_202403.json, тот же адрес/девелопер (Coldy), что "Nice Tower"
}

# Строки-артефакты парсинга исходных Excel-сводов (итоговая строка или
# битая объединённая ячейка), а не проекты и не классы. Найдены при
# полной сверке 140 ключей data/lots_*.json (2021-2026) — все из
# lots_202403.json, все с price=0 и нечисловыми block/floor вместо
# нормальных значений. Кириллические "А"/"В+" — визуальные омоглифы
# латинских class-меток A/B+, тоже не проекты.
NON_PROJECT_SALE_LOT_ARTIFACTS = {"Общий итог", "А", "В+"}
